...
\begin{eqnarray}
	...
	\mathbf{F}(\mathbf{x}_1, \mathbf{x}_2) &=& \begin{pmatrix} \frac{1}{2}(x_{1,1} + x_{2,1}) \\
		\frac{1}{2}(x_{1,2} + x_{2,2}) \\  \arcsin{\frac{x_{2,1}-x_{1,1}}{\sqrt{(x_{1,1}-x_{2,1})^{2}+(x_{1,2}-x_{2,2})^{2}}}}\end{pmatrix} \label{eq:F}
\end{eqnarray}

To propagate the error from measuremnts of $x_1$ and $x_2$ the Jacobian matrix $\mathbf{J}$ needs to be calculated. Here, the Jacobian matrix $\mathbf{J}$ is a $2 \times 4$ matrix which can be seen in Equation \ref{eq:Jac}.
