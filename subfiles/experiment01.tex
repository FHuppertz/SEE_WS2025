...
Let $\mathbf{F}$ be a vector function, defining the \textbf{end pose} of the robot, calculated from two measured positions on a 2D plane, $\mathbf{x}_1$ and $\mathbf{x}_2$. The output components are defined such that $\mathbf{F}_1$ and $\mathbf{F}_2$ represent the position coordinates and $\mathbf{F}_2$ represents the angle of the end pose. The input vectors and the functional relationship are defined in Equations \ref{eq:x1x2} to \ref{eq:F}.
...
\begin{eqnarray}
	...
	\mathbf{F}(\mathbf{x}_1, \mathbf{x}_2) &=& \begin{pmatrix}
		...
		\arctan\left(\frac{x_{1,1}-x_{2,1}}{x_{1,2}-x_{2,2}}\right)
