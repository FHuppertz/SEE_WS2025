...
The provided camera was tested to ensure proper connectivity and stability during image capture. 
Auto-focus was \textit{(disabled as required)}, and the resolution was set to the highest possible. 
...
A total of \textit{(18)} images were captured at varying orientations{(straight, diagonal, tilted clockwise, tilted anticlockwise and many more)} and distances of the checkerboard. 
The board was moved to different positions and angles to provide sufficient geometric diversity for accurate calibration. 
Certain frames were \textit{(discarded/retained)} due to \textit{(blur, low contrast, poor corner detection, etc.)}. 
