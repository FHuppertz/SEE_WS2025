When we use the Chebishov function to remove the outliers with using the threshold as 2.0 standard deviations, we were able to remove 7 outliers in total out of the 75 manual readings that we measured. For reference we are attaching a screenshot of the results from the code we ran to find this.
...
We could very well see which graphs are fitting into the Gaussian defenition and which ones are not. The Left and Right robot movements are not Gaussian at all meanwhile the Forward movement is a perfect Gaussian fit.
